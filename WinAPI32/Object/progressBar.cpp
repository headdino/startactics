...
progressBar::progressBar() : parent(nullptr)
{
	...
	vertices[0].Position = D3DXVECTOR3(0.0f, 0.0f, 0.5f);
	vertices[1].Position = D3DXVECTOR3(0.0f, 1.0f, 0.5f);
	vertices[2].Position = D3DXVECTOR3(1.0f, 0.0f, 0.5f);
	vertices[3].Position = D3DXVECTOR3(1.0f, 0.0f, 0.5f);
	vertices[4].Position = D3DXVECTOR3(0.0f, 1.0f, 0.5f);
	vertices[5].Position = D3DXVECTOR3(1.0f, 1.0f, 0.5f);
	...
	vertices2[0].Position = D3DXVECTOR3(0.0f, 0.0f, 0.5f);
	vertices2[1].Position = D3DXVECTOR3(1.0f, 0.0f, 0.5f);
	vertices2[2].Position = D3DXVECTOR3(1.0f, 1.0f, 0.5f);
	vertices2[3].Position = D3DXVECTOR3(0.0f, 1.0f, 0.5f);
	vertices2[4].Position = D3DXVECTOR3(0.0f, 0.0f, 0.5f);
...
void progressBar::Position(float x, float y)
{
	Position(D3DXVECTOR2(x, y));
}

void progressBar::Position(D3DXVECTOR2 vec)
{
	position = vec;
}

void progressBar::Scale(float x, float y)
{
	Scale(D3DXVECTOR2(x, y));
}

void progressBar::Scale(D3DXVECTOR2 vec)
{
	scale = vec;
}

void progressBar::Rotate(D3DXVECTOR3 x)
{
	angle = x;
}
void progressBar::Rotate(float x, float y, float z)
{
	Rotate(D3DXVECTOR3(x, y, z));
}
void progressBar::Rotate2(D3DXVECTOR3 x)
{
	angle2 = x;
}
void progressBar::Rotate2(float x, float y, float z)
{
	Rotate2(D3DXVECTOR3(x, y, z));
}

void progressBar::Color(float r, float g, float b, float a)
{
	Color(D3DXCOLOR(r, g, b, a));
}

void progressBar::Color(D3DXCOLOR vec)
{
	colorBufferData.Color = vec;
}

void progressBar::Direction(DIRECTION dirtion)
{
	dir = dirtion;
}
...
void progressBar::Update()
{
	...
	case DIR_SW:
		...
		//rc = RectMake(position.x, position.y, -scale.x, -scale.y);
		break;
	case DIR_SE:
		D3DXMatrixScaling(&S, scale.x, -scale.y, 1);
		//rc = RectMake(position.x, position.y, scale.x, -scale.y);
	...
	rc = RectMake(position.x, position.y, scale.x, scale.y, dir);
	if (parent)
	{
		W *= *parent;
		RT *= *parent;
